...
\begin{figure} % wrapfigure is too small to see anything useful
    \includegraphics[width=\textwidth]{img/schema.png} % You should use a PNG at >= 300 dpi; not a lossy format like JPEG. You can export PNGs from PDFs using Preview
...
So what use is this provenance metadata, if as shown in fig. \ref{fig:sub-provenance} it is so difficult to read or visualise? Imagine for the purposes of illustration that
...
rather than the relational database. In addition we used the workflow sub-graph because this is probably one of the easier graphs to follow visually. 


\begin{figure} 
    \includegraphics[width=\textwidth]{img/broken-application.png}
    \caption{The workflow sub-graph show the propagation of a broken application (in red) in the background. The break-out box shows a magnified detail to show the content of the graph.} 
    \label{fig:broken-application} 
\end{figure}
